...
FILE_DIR = '/Users/xiw4021/Downloads/WebOfScience/Meta-data/Data.txt'
total_len = []
np.random.seed(7)
...
def get_data_from_meta():
    ...
    for line in origin_txt[1:]:
        ...
        data.append({'abstract': sample_text, 'label': sample_label, 'topic': [], 'keyword': []})
    print(label_check)
    c = 0
    ...
    f = open('data/WOS/wos_total.json', 'w')
    for line in data:
...
def split_train_dev_test():
    ...
    val = list(val)
    test = list(test)
    f = open('data/WOS/wos_train.json', 'w')
    f.writelines(train)
    f.close()
    f = open('data/WOS/wos_test.json', 'w')
    f.writelines(test)
    f.close()
    f = open('data/WOS/wos_val.json', 'w')
    f.writelines(val)
    f.close()
...
def get_hierarchy():
    ...
    f = open('data/WOS/wos.taxonomy', 'w')
...
def get_label_list():
    ...
    # Iterate through the dataset and add each label to the set
    for item in data:
        item = json.loads(item)
        labels = item['label']  # Get the list of labels for the current item
        unique_labels.update(labels)  # Add each label to the set

    unique_labels = list(unique_labels)
    with open('data/WOS/labels.txt', 'w') as file:
        # Iterate over each string in the list and write to the file
